...
class HandsDetectionTransformer:
    def __init__(self):
    ...
    async def recv_queued(self, frames):
        ...
        if img is not None:    
            ...
            with self.mp_hands.Hands(static_image_mode=True) as hands:
                ...
                # Process image.
                results = hands.process(annotated_img)
...
def app():
    ...
    st.title("Hands Detection from live video stream")
    ...
    if MultiPage.localhost:
        ...
        rtc_context = webrtc_streamer(key="face_video", video_processor_factory=HandsDetectionTransformer, media_stream_constraints={"video": True, "audio": False})
    else:
        ...
        rtc_context = webrtc_streamer(key="face_video", mode=WebRtcMode.SENDRECV, rtc_configuration=RTC_CONFIGURATION,video_processor_factory=HandsDetectionTransformer, media_stream_constraints={"video": True, "audio": False})
