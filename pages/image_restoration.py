...
def app():
    ...
    # Set title.
    st.title('Image Restoration with Inpaint')
    ...
    if uploaded_file is not None:
        ...
        inpaint_radius = st.sidebar.slider("Inpaint radius: ", 1, 25, 3)
        ...
        if option == 'Telea':
            ...
            res = cv2.inpaint(src=image, inpaintMask=mask, inpaintRadius=inpaint_radius, flags=cv2.INPAINT_TELEA)[:,:,::-1]
            st.image(res)
        elif option == 'Compare both':
            col1, col2 = st.columns(2)
            res1 = cv2.inpaint(src=image, inpaintMask=mask, inpaintRadius=inpaint_radius, flags=cv2.INPAINT_TELEA)[:,:,::-1]
            res2 = cv2.inpaint(src=image, inpaintMask=mask, inpaintRadius=inpaint_radius, flags=cv2.INPAINT_NS)[:,:,::-1]
            with col1:
                st.subheader('Result of Telea')
                st.image(res1)
            with col2:
            ...
            st.subheader('Result of NS')
            res = cv2.inpaint(src=image, inpaintMask=mask, inpaintRadius=inpaint_radius, flags=cv2.INPAINT_NS)[:,:,::-1]
            st.image(res)
        else:
            pass

        if res is not None and option != 'Compare both':
            # Display link.
